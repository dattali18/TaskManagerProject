...
struct CoursesView: View {
    ...
    @StateObject var viewModel: CoursesViewModel = CoursesViewModel()
                ...
                ForEach(Array(viewModel.sectionsName.enumerated()), id: \.element) { index, name in
                    Section {
                        ...
                        if name != viewModel.noSemester {
                            NavigationLink(
                                destination: SemesterView(semester: viewModel.getSemesterByName(name: name)))
                            {
                                HStack {
                                    Text(name)
                                    Spacer()
            ...
            .sheet(isPresented: $viewModel.isPresented, onDismiss: {
                ...
                if let action = viewModel.action {
                    switch action {
                        ...
                        AddEditCourseView()
                        case .editCourse:
                        AddEditCourseView(with: viewModel.course)
                        case .addSemester:
                        AddEditSemesterView()
                        case .editSemester:
                        AddEditSemesterView(with: viewModel.semester)
                    }
                }
            }
            .toolbar {
